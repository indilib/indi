...
if [ ! -z $BUILD_THIRD_PARTY ]; then
  ...
  if [ ${TRAVIS_OS_NAME} == "linux" ] ; then 
    ...
    cmake -DCMAKE_INSTALL_PREFIX=/usr/local/ -DWITH_NSE:OPTION=ON -DWITH_MI:OPTION=OFF -DWITH_QHY:OPTION=OFF -DWITH_SBIG:OPTION=OFF -DWITH_APOGEE:OPTION=OFF -DWITH_FLI:OPTION=OFF -DWITH_EQMOD:OPTION=OFF . ../../3rdparty/
  fi
  sudo make install
  popd
else
