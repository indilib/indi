...
bool FLICFW::setupParams()
{
    ...
    MinFilter = 1;
    MaxFilter = FLIFilter.count;

    FilterSlotN[0].min = MinFilter;
...
bool FLICFW::GetFilterNames(const char* groupName)
{
    ...
    IUFillTextVector(FilterNameTP, FilterNameT, FLIFilter.count, getDeviceName(), "FILTER_NAME", "Filter", groupName, IP_RW, 0, IPS_IDLE);
...
void FLICFW::turnWheel()
{
        ...
        {
          case ISS_ON:
            if (current_filter < MaxFilter)
                current_filter++;
            else current_filter = MinFilter;
            break;

          case ISS_OFF:
            if (current_filter > MinFilter)
                current_filter--;
            else current_filter = MaxFilter;
            break;
        }
