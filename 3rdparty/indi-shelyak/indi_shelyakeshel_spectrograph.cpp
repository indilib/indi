...
std::unique_ptr<ShelyakEshel> shelyakEshel(new ShelyakEshel()); // create std:unique_ptr (smart pointer) to  our spectrograph object
...
bool ShelyakEshel::initProperties()
{
  ...
  IUFillSwitchVector(&MirrorSP, MirrorS, 2, getDeviceName(), "FLIP_MIRROR", "Flip mirror", CALIBRATION_UNIT_TAB, IP_RW, ISR_1OFMANY, 0, IPS_IDLE);
  ...
  IUFillNumberVector(&SettingsNP, SettingsN, 5, getDeviceName(), "SPECTROGRAPH_SETTINGS", "Spectrograph settings", SPECTROGRAPH_SETTINGS_TAB, IP_RW, 60, IPS_IDLE);
...
bool ShelyakEshel::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
  if (!strcmp(dev, getDeviceName())) // check if the message is for our device
  {
    ...
    if (!strcmp(MirrorSP.name, name)) // check if its a mirror request
    {
      ...
      if (s != MirrorS[0].s) { // if mirror state has changed send command
        bool rc = calibrationUnitCommand(COMMANDS[MirrorS[0].s],PARAMETERS["MIRROR"]);
        if (!rc) MirrorSP.s = IPS_ALERT;
      }
...
bool ShelyakEshel::calibrationUnitCommand(char command, char parameter)
{
  ...
  char c[5] = {0x0D,0x01,command,parameter}; // the first 2 bytes are constant, the next 2 are command and parameter
