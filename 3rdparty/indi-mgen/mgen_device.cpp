...
int MGenDevice::Connect(unsigned short vid, unsigned short pid)
{
    ...
    if(lock())
    {
        ...
        _S("Connecting device %04X:%04X using FTDI %s v%d.%d.%d snapshot %s", vid, pid, ftdi_version.version_str, ftdi_version.major, ftdi_version.minor, ftdi_version.micro, ftdi_version.snapshot_str);
        ...
        else if((res = ftdi_usb_open(ftdi, vid, pid)) < 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else _S("no FTDI device enumerated","");
...
int MGenDevice::write(IOBuffer const & query) throw (IOError)
{
    ...
    _D("writing %d bytes to device: %02X %02X %02X %02X %02X ...", query.size(), query.size()>0?query[0]:0, query.size()>1?query[1]:0, query.size()>2?query[2]:0, query.size()>3?query[3]:0, query.size()>4?query[4]:0);
...
int MGenDevice::read(IOBuffer & answer) throw (IOError)
{
    ...
    if(answer.size() > 0)
    {
        ...
        _D("read %d bytes from device: %02X %02X %02X %02X %02X ...", bytes_read, answer.size()>0?answer[0]:0, answer.size()>1?answer[1]:0, answer.size()>2?answer[2]:0, answer.size()>3?answer[3]:0, answer.size()>4?answer[4]:0);
        return bytes_read;
    }
