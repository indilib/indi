...
#include "focuserserver.h"
#include "focuserserverclient.h"
#include "indicom.h"

#define POLLMS  100

std::unique_ptr<FocuserServer> fs(new FocuserServer());

void ISGetProperties(const char * dev)
{
    fs->ISGetProperties(dev);
}

void ISNewSwitch(const char * dev, const char * name, ISState * states, char * names[], int num)
{
    fs->ISNewSwitch(dev, name, states, names, num);
}

void ISNewText(	const char * dev, const char * name, char * texts[], char * names[], int num)
{
    fs->ISNewText(dev, name, texts, names, num);
}

void ISNewNumber(const char * dev, const char * name, double values[], char * names[], int num)
{
    fs->ISNewNumber(dev, name, values, names, num);
}
...
{
    fs->ISSnoopDevice(root);
}

FocuserServer::FocuserServer()
{
    setVersion(0, 1);
...
const char * FocuserServer::getDefaultName()
{
    return (char *)"FocuserServer";
}

bool FocuserServer::Connect()
{
    bool rc = startServer();
    if (rc)
    {
...
bool FocuserServer::Disconnect()
{
    return stopServer();
}

bool FocuserServer::initProperties()
{
    ...
    IUFillText(&SettingsT[FOCUSER_SERVER_PORT], "FOCUSER_SERVER_PORT", "Focuser Server port", "9624");
...
void FocuserServer::ISGetProperties (const char * dev)
{
...
bool FocuserServer::ISNewText (const char * dev, const char * name, char * texts[], char * names[], int n)
{
...
bool FocuserServer::ISNewNumber (const char * dev, const char * name, double values[], char * names[], int n)
{
    return DefaultDevice::ISNewNumber(dev, name, values, names, n);
}

bool FocuserServer::ISNewSwitch (const char * dev, const char * name, ISState * states, char * names[], int n)
{
    if(strcmp(dev, getDeviceName()) == 0)
    {
...
bool FocuserServer::saveConfigItems(FILE * fp)
{
...
void FocuserServer::TimerHit()
{
    ...
    if (clientFD == -1)
    {
        ...
        {
            DEBUGF(INDI::Logger::DBG_ERROR, "Failed to connect to Focuser Server. %s", strerror(errno));
            SetTimer(POLLMS);
            return;
        }
        ...
        {
            DEBUGF(INDI::Logger::DBG_ERROR, "Error connecting to Focuser Server. F_GETFL: %s", strerror(errno));
        }

        // Set to Non-Blocking
        if (fcntl(clientFD, F_SETFL, flags | O_NONBLOCK) < 0)
        {
            DEBUGF(INDI::Logger::DBG_ERROR, "Error connecting to Focuser Server. F_SETFL: %s", strerror(errno));
        }

        // Only show message first time Focuser Server connects
        if (isSkySafariConnected == false)
        {
            DEBUG(INDI::Logger::DBG_SESSION, "Connected to Focuser Server.");
            isSkySafariConnected = true;
        }
    }
    else
    {
        // Read from Focuser Server
        char buffer[64] = { 0 };
        ...
        {
            //DEBUG(INDI::Logger::DBG_ERROR, "Focuser Server Disconnected? Reconnect again.");
            close(clientFD);
            clientFD = -1;
        }
...
bool FocuserServer::startServer()
{
    ...
    serv_socket.sin_port = htons ((unsigned short)atoi(SettingsT[FOCUSER_SERVER_PORT].text));
    ...
    lsocket = sfd;
    DEBUG(INDI::Logger::DBG_SESSION, "Focuser Server Server is running. Connect the App now to this machine using Focuser Server LX200 driver.");
    return true;
}

bool FocuserServer::stopServer()
{
    if (clientFD > 0)
        close(clientFD);
    if (lsocket > 0)
...
void FocuserServer::processCommand(std::string cmd)
{
...
void FocuserServer::sendGeographicCoords()
{
...
bool FocuserServer::sendSkySafari(const char * message)
{
    ...
    while (bytesWritten < totalBytes)
    {
        ...
        {
            DEBUGF(INDI::Logger::DBG_ERROR, "Error writing to Focuser Server. %s", strerror(errno));
            return false;
        }
    }
...
void FocuserServer::sendUTCtimedate()
{
...
template<typename Out> void FocuserServer::split(const std::string &s, char delim, Out result)
{
    std::stringstream ss;
    ss.str(s);
    std::string item;
...
std::vector<std::string> FocuserServer::split(const std::string &s, char delim)
{
