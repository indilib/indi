...
bool SBIGCCD::initProperties()
{
    ...
    IUFillSwitchVector(&CoolerSP, CoolerS, 2, getDeviceName(), "CCD_COOLER", "Cooler", MAIN_CONTROL_TAB, IP_RW, ISR_1OFMANY, 0, IPS_OK);


    // CCD COOLER:
    IUFillNumber(	&CoolerN[0], "CCD_COOLER_VALUE","[%]", "%.1f", 0, 0, 0, 0);
    IUFillNumberVector(&CoolerNP, CoolerN, 1, getDeviceName(), "CCD_COOLER_POWER","Cooler %", MAIN_CONTROL_TAB, IP_RO, 0, IPS_IDLE);
...
bool SBIGCCD::setupParams()
{
    ...
    if (HasGuideHead())
    {
        ...
        if (getCCDSizeInfo(useExternalTrackingCCD ? CCD_EXT_TRACKING : CCD_TRACKING, binning, wCcd, hCcd, wPixel, hPixel) != CE_NO_ERROR)
        {
...
int SBIGCCD::StartExposure(CCDChip *targetChip, double duration)
{
    ...
    DEBUGF(INDI::Logger::DBG_DEBUG, "Exposure params for CCD (%d) openShutter(%d), exposureTime (%ld), binnig (%d), left (%d), top (%d), w(%d), h(%d)", sep.ccd, sep.openShutter, sep.exposureTime, sep.readoutMode, sep.left, sep.top, sep.width, sep.height);
...
bool SBIGCCD::grabImage(CCDChip *targetChip)
{
    ...
    unsigned short width	= (unsigned short)targetChip->getSubW()/targetChip->getBinX();
    unsigned short height	= (unsigned short)targetChip->getSubH()/targetChip->getBinY();
    DEBUGF(INDI::Logger::DBG_DEBUG, "%s readout in progress...", targetChip == &PrimaryCCD ? "Primary camera":"Guide head");
    if (isSimulation())
    {
    ...
    else
    {
        ...
        if (res != CE_NO_ERROR)
        {
            DEBUGF(INDI::Logger::DBG_ERROR, "%s readout error", targetChip == &PrimaryCCD ? "Primary camera":"Guide head");
            return false;
        }
    }
