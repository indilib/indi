...
// 			src_pixel=(row*src_width*2+col*2);//*2; //Bayer pattern has 2 bytes in first row: GR
// 			src_pixel_row2=((row+1)*src_width*2+col*2);//*2; //Bayer pattern has 2 bytes in 2nd row:BG
			//src_pixel=(row*src_width+col);
			//src_pixel_row2=((row+1)*src_width+col);
// 			dst[i]/*RED*/=src[src_pixel+1];
// 			dst[i+1]/*GREEN*/=(src[src_pixel+0]+src[src_pixel_row2+1])/2;
// 			dst[i+2]/*BLUE*/=src[src_pixel_row2+0];
			} else {
			//Experimental	
// 				dst[(row*width+col)*3+RED]=;
// 				dst[(row*width+col)*3+GREEN]=;
// 				dst[(row*width+col)*3+BLUE]=;
				//General case:
