...
bool AstrometryDriver::initProperties()
{
    ...
    IUFillSwitchVector(&SolverSP, SolverS, 2, getDeviceName(), "ASTROMETRY_SOLVER", "Solver", MAIN_CONTROL_TAB, IP_RW, ISR_1OFMANY, 0, IPS_IDLE);
...
bool AstrometryDriver::processBLOB(const uint8_t *data, const uint32_t size)
{
    ...
    int result = pthread_create( &solverThread, NULL, &AstrometryDriver::runSolverHelper, this);
