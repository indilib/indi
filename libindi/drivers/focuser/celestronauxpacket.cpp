...
void Packet::FillBuffer(buffer &buff)
{
    ...
    DEBUGFDEVICE(Communicator::Device.c_str(), INDI::Logger::DBG_DEBUG, "fillBuffer <%s>", buff.data());
}
...
bool Communicator::readPacket(int portFD, Packet &reply)
{
    ...
    DEBUGFDEVICE(Communicator::Device.c_str(), INDI::Logger::DBG_DEBUG, "RES <%s>", packet.data());
