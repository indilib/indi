...
int check_IEQ45_connection(int in_fd)
{
  ...
  for (i=0; i < 2; i++)
  {
    if (write(in_fd,  firmwareVersion, sizeof(firmwareVersion)) < 0) return -1;
  ...
  for (i=0; i < 2; i++)
  {
    if (write(in_fd,  MountInfo, sizeof(MountInfo)) < 0) return -1;
    tty_read(in_fd, response, 1, IEQ45_TIMEOUT, &nbytes_read);
    if (nbytes_read == 1)
     return 0;
    usleep(50000);
...
int SendPulseCmd(int fd, int direction, int duration_msec)
{
  ...
  switch (direction)
  {
    case IEQ45_NORTH: sprintf(cmd, ":Mgn%04d#", duration_msec); break;
    case IEQ45_SOUTH: sprintf(cmd, ":Mgs%04d#", duration_msec); break;
    case IEQ45_EAST:  sprintf(cmd, ":Mge%04d#", duration_msec); break;
    case IEQ45_WEST:  sprintf(cmd, ":Mgw%04d#", duration_msec); break;
    default: return 1;
  }
