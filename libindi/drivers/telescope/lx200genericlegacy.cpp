...
LX200GenericLegacy *telescope = NULL;
...
void ISInit()
{
 ...
 {
  telescope = new LX200GenericLegacy();
...
void LX200GenericLegacy::setCurrentDeviceName(const char * devName)
{
  strcpy(thisDevice, devName);
}

void LX200GenericLegacy::ISGetProperties(const char *dev)
{
...
void LX200GenericLegacy::ISSnoopDevice (XMLEle *root)
{
  INDI_UNUSED(root);
}

void LX200GenericLegacy::ISNewText (const char *dev, const char *name, char *texts[], char *names[], int n)
{
	int err;
	IText *tp;
...
void LX200GenericLegacy::ISNewNumber (const char *dev, const char *name, double values[], char *names[], int n)
{
	int h =0, m =0, s=0, err;
...
void LX200GenericLegacy::ISNewSwitch (const char *dev, const char *name, ISState *states, char *names[], int n)
{
	int err=0, index=0;
	INDI_UNUSED(names);
	...
	if (!strcmp (name, FocusMotionSP.name))
	{
	  ...
	  if (FocusTimerN[0].value > 0)  
	  {
         ...
         IEAddTimer(50, LX200GenericLegacy::updateFocusTimer, this);
	  }
...
void LX200GenericLegacy::handleError(ISwitchVectorProperty *svp, int err, const char *msg)
{
...
void LX200GenericLegacy::handleError(INumberVectorProperty *nvp, int err, const char *msg)
{
...
void LX200GenericLegacy::handleError(ITextVectorProperty *tvp, int err, const char *msg)
{
 ...
 void LX200GenericLegacy::correctFault()
 {
...
bool LX200GenericLegacy::isTelescopeOn(void)
{
...
void LX200GenericLegacy::updateFocusTimer(void *p)
{
   int err=0;
    ...
    switch (FocusTimerNP.s)
    {
	  ...
	  if (FocusTimerN[0].value > 0)
        IEAddTimer(50, LX200GenericLegacy::updateFocusTimer, p);
	    break;
...
void LX200GenericLegacy::guideTimeout(void *p)
{
...
void LX200GenericLegacy::ISPoll()
{
    double dx, dy;
	int err=0;
...
// wildi nothing changed in LX200GenericLegacy::mountSim
void LX200GenericLegacy::mountSim ()
{
	...
	case IPS_BUSY:
	    ...
	    {
		EquatorialCoordsRNP.s = IPS_OK;
        IDSetNumber(&EquatorialCoordsRNP, "Now tracking");
	    } else
		IDSetNumber(&EquatorialCoordsRNP, NULL);
...
void LX200GenericLegacy::getBasicData()
{
...
int LX200GenericLegacy::handleCoordSet()
{
...
int LX200GenericLegacy::getOnSwitch(ISwitchVectorProperty *sp)
{
...
int LX200GenericLegacy::checkPower(ISwitchVectorProperty *sp)
{
...
int LX200GenericLegacy::checkPower(INumberVectorProperty *np)
{
  if (simulation) return 0;
...
void LX200GenericLegacy::connectTelescope()
{
...
void LX200GenericLegacy::slewError(int slewCode)
{
    EquatorialCoordsRNP.s = IPS_ALERT;

    if (slewCode == 1)
    IDSetNumber(&EquatorialCoordsRNP, "Object below horizon.");
    else if (slewCode == 2)
    IDSetNumber(&EquatorialCoordsRNP, "Object below the minimum elevation limit.");
    else
    IDSetNumber(&EquatorialCoordsRNP, "Slew failed.");

}

void LX200GenericLegacy::getAlignment()
{
...
void LX200GenericLegacy::enableSimulation(bool enable)
{
   simulation = enable;
...
void LX200GenericLegacy::updateTime()
{
  #ifdef HAVE_NOVA_H
  char cdate[32];
  double ctime;
...
void LX200GenericLegacy::updateLocation()
{
