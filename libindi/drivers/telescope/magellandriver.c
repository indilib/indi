...
int check_magellan_connection(int fd)
{
  ...
  if (fd <= 0) return MAGELLAN_ERROR;

  for (i=0; i < CONNECTION_RETRIES; i++)
  {
    if (write(fd, ack, 4) < 0) return MAGELLAN_ERROR;
    
    tty_read(fd, Response, 2, MAGELLAN_TIMEOUT, &nbytes_read);
    
    if (nbytes_read == 2) return MAGELLAN_OK;
...
int getCalenderDate(int fd, char *date)
{
 ...
 if ( (result = getCommandString(fd, date, "#:GC#")) )
 {
	...
	in the 21st century.*/
      if (yy > CENTURY_THRESHOLD) strncpy(century, "19", 3);
      else strncpy(century, "20", 3);
