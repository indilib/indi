...
int STV_AKey(void) { /* Parameter button */
  int data[]={A_KEY_PATTERN} ;
  return STV_SendPacket(SEND_KEY_PATTERN, data, 1) ;
}

int STV_BKey(void) { /* Value Button */
...
int STV_RequestImageData( int compression, int *data, int j, int length, int *values) {
  ...
  if( compression== ON) { /* compressed download */
...
int STV_DecompressData( unsigned char *data, int *values, int length, int expected_n_values) {
  ...
  while( i < length ) {
    ...
    if( (( data[ i] & 0xc0))== 0x80) { /*two bytes, first byte: bit 7 set, 6 cleared, 14 bits data */

      if(( data[ i] & 0x20)== 0x20) { /* minus sign set? */

	value= - ((int)(( (~data[ i] & 0x1f)) * 0x100) + (((int) (~data[ i+ 1])) & 0xff))- 1 ; /* value without sign */
      } else {
	...
	value=   (int) (( data[ i] & 0x1f) * 0x100) + (int) (data[ i + 1]) ;
      ...
      if(( data[ i] & 0x40)== 0x40) { /* minus sign set? */
...
int STV_ReceivePacket( unsigned char *buf, int mode) {
  ...
  if(( buf[0]== 0xa5) && ( pos== 6)) { /* Check the sanity of the header part */
    if((res= STV_CheckHeaderSum( buf))== 0) {
      ...
      if( length >0) {
        ...
        trb= 0 ;
	while( pos < 6 + length + 2) { /* header, data, check sum */
  ...
  if( length > 0) { /* Check the sanity of the data */
...
int STV_SendPacket( int cmd, int *data, int n) {
  ...
  if(( res= STV_CheckHeaderSum( (unsigned char *) buf)) != 0) { /* Check outgoing packet as well */
...
unsigned int STV_GetBits( unsigned int x, int p, int n){ /* from the C book */
...
double STV_SetCCDTemperature( double set_value) {
  ...
  for( i=0 ; i< 100  ; i++) { /* Change to the highest temperature */
...
int STV_SetDateTime( char *times) 
{
  ...
  for( i=0 ; i< 13  ; i++) { /* Reset Month menu to the lef most position */
  ...
  for( i=0 ; i< 32 ; i++) { /* Reset Day menu to the lef most position */
    res= STV_LRRotaryDecrease() ; 
    usleep( delay) ;
    tcflush(fd, TCIOFLUSH);
  }
  ...
  for( i=0 ; i< 128  ; i++) { /* Reset Year menu to the lef most position, ATTENTION */
    res= STV_LRRotaryDecrease() ; 
 ...
 tcflush(fd, TCIOFLUSH);
  ...
  for( i=0 ; i< 25  ; i++) { /* Reset Hour menu to the lef most position, ATTENTION */
    res= STV_LRRotaryDecrease() ; 
    usleep( delay) ; 
    tcflush(fd, TCIOFLUSH);
  }
  ...
  for( i=0 ; i< 61  ; i++) { /* Reset Minute menu to the lef most position, ATTENTION */
    res= STV_LRRotaryDecrease() ; 
    usleep( delay) ;
    tcflush(fd, TCIOFLUSH);
  }
  ...
  res=  STV_AKey() ; /* Press the Parameter button */
  tcflush(fd, TCIOFLUSH); 
  for( i=0 ; i< 5  ; i++) { /* Reset Seconds menu to the lef most position, ATTENTION */
    res= STV_LRRotaryDecrease() ; 
    usleep( delay) ; 
    tcflush(fd, TCIOFLUSH);
  }
  ...
  for( i=0 ; i< turn  ; i++) { /* Set Seconds menu, steps of 15 seconds */
    res= STV_LRRotaryIncrease() ;
    usleep( delay) ;
    tcflush(fd, TCIOFLUSH);
  }
...
int STV_MenueDateTime( int delay) {
  ...
  for( i=0 ; i< 8  ; i++) { /* Reset Date menu to the lef most position */
...
int STV_MenueSetup( int delay) {
  ...
  for( i=0 ; i< 16  ; i++) { /* Reset Setup menu to the lef most position */
    res= STV_UDRotaryDecrease() ;
    usleep( delay) ;
    tcflush(fd, TCIOFLUSH);
  }
 ...
 *****************************************************************************/
int
init_serial(char *device_name, int bit_rate, int word_size,
