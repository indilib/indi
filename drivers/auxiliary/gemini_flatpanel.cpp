...
bool GeminiFlatpanel::sendCommand(const char *command, char *response, int timeout)
{
    ...
    if ((rc = tty_nread_section(PortFD, response, MAXRBUF, '#', timeout, &nbytes_read)) != TTY_OK)
    {
        char errstr[MAXRBUF] = {0};
        tty_error_msg(rc, errstr, MAXRBUF);
    ...
    if (nbytes_read > 0 && response[nbytes_read - 1] == '\r')
    {
...
bool GeminiFlatpanel::getConfigStatus()
{
    ...
    int status;
    if (!extractIntValue(response, 2, &status))
    {
        return false;
    }

    configStatus = status ? GEMINI_CONFIG_READY : GEMINI_CONFIG_NOTREADY;
    return true;
}
...
bool GeminiFlatpanel::setBrightness(int value)
{
    ...
    snprintf(command, MAXRBUF, ">B%03d#", value);
...
bool GeminiFlatpanel::lightOn()
{
    ...
    if (!sendCommand(">L#", response))
    {
        return false;
    }

    return (strlen(response) >= 4 && response[0] == '*' && response[1] == 'L');
}
...
bool GeminiFlatpanel::lightOff()
{
    ...
    if (!sendCommand(">D#", response))
    {
        return false;
    }

    return (strlen(response) >= 4 && response[0] == '*' && response[1] == 'D');
}
...
{
    char command[MAXRBUF] = {0};
    snprintf(command, MAXRBUF, ">T%d#", enable ? GEMINI_BEEP_ON : GEMINI_BEEP_OFF);
...
bool GeminiFlatpanel::setBrightnessMode(int mode)
{
    ...
    char command[MAXRBUF] = {0};
    snprintf(command, MAXRBUF, ">Y%d#", mode);
