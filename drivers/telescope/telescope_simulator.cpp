...
/// ALTAZ: The tracking rates of the mechanical axes vary with the positions.
...
bool ScopeSim::ReadScopeStatus()
{
    ...
    {
        double sinAz, sinAlt, cosAz, cosAlt;
        double basetrackrate = TRACKRATE_SIDEREAL;

        sinAz = std::sin(m_currentAz * 0.0174533);
        cosAz = std::cos(m_currentAz * 0.0174533);
        sinAlt = std::sin(m_currentAlt * 0.0174533);
        cosAlt = std::cos(m_currentAlt * 0.0174533);
        double primtrackrate = (m_sinLat - ((cosAz * sinAlt * m_cosLat) / cosAlt)) * basetrackrate;
        double sectrackrate = m_cosLat * sinAz * basetrackrate;
        SetTrackRate(primtrackrate, sectrackrate);
    }
