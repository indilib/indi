...
bool DomePro2::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        {
            HomeSP.reset();
            for (size_t i = 0; i < HomeSP.size(); i++)
            {
                if (states[i] != ISS_ON)
                    continue;

                if (!strcmp(HomeSP[HOME_GOTO].getName(), names[i]))
                {
                    if (!gotoHomeDomeAz())
                    {
                ...
                else if (!strcmp(HomeSP[HOME_DISCOVER].getName(), names[i]))
                {
...
bool DomePro2::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        {
            bool allSet = true;
            for (size_t i = 0; i < SettingsNP.size(); i++)
            {
                if (!strcmp(SettingsNP[SETTINGS_AZ_CPR].getName(), names[i]))
                {
                ...
                else if (!strcmp(SettingsNP[SETTINGS_AZ_COAST].getName(), names[i]))
                {
                ...
                else if (!strcmp(SettingsNP[SETTINGS_AZ_HOME].getName(), names[i]))
                {
                ...
                else if (!strcmp(SettingsNP[SETTINGS_AZ_PARK].getName(), names[i]))
                {
                ...
                else if (!strcmp(SettingsNP[SETTINGS_AZ_STALL_COUNT].getName(), names[i]))
                {
...
void DomePro2::TimerHit()
{
            ...
            strcmp(shutterStatus.c_str(), StatusTP[STATUS_DOME].getText()) != 0))
    {
        if (getDomeState() == DOME_MOVING || getDomeState() == DOME_PARKING)
        {
            ...
            if (!strcmp(StatusTP[STATUS_DOME].getText(), "Idle"))
            {
...
{
    INDI_UNUSED(az);
    uint32_t steps = static_cast<uint32_t>(az * (SettingsNP[SETTINGS_AZ_CPR].value / 360));
    if (gotoDomeAz(steps))
        return IPS_BUSY;
    return IPS_ALERT;
}
...
{
    if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "Opened"))
        return 0;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "Closed"))
        return 1;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "Opening"))
        return 2;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "Closing"))
        return 3;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "ShutterError"))
        return 4;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter module is not communicating to the azimuth module"))
        return 5;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 opposite direction timeout error on open occurred"))
        return 6;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 opposite direction timeout error on close occurred"))
        return 7;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 opposite direction timeout error on open occurred"))
        return 8;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 opposite direction timeout error on close occurred"))
        return 9;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 completion timeout error on open occurred"))
        return 10;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 completion timeout error on close occurred"))
        return 11;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 completion timeout error on open occurred"))
        return 12;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 completion timeout error on close occurred"))
        return 13;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 limit fault on open occurred"))
        return 14;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 limit fault on close occurred"))
        return 15;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 limit fault on open occurred"))
        return 16;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 limit fault on close occurred"))
        return 17;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "Shutter disabled (Shutter Enable input is not asserted)"))
        return 18;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "Intermediate"))
        return 19;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "GoTo"))
        return 20;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 OCP trip on open occurred"))
        return 21;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 1 OCP trip on close occurred"))
        return 22;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 OCP trip on open occurred"))
        return 23;
    else if (!strcmp(StatusTP[STATUS_SHUTTER].getText(), "shutter 2 OCP trip on close occurred"))
        return 24;
    return 25;
}
...
bool DomePro2::getDomeStatus()
{
    ...
    {
        StatusTP[STATUS_DOME].setText(DomeStatus.at(res));
        return true;
    }
    else
    {
...
bool DomePro2::setDomeParkAz(uint32_t az)
{
    char cmd[DRIVER_LEN] = {0};
    uint32_t steps = static_cast<uint32_t>(az * (SettingsNP[SETTINGS_AZ_CPR].value / 360));
    //    if (park >= SettingsNP[SETTINGS_AZ_CPR].getValue())
    //    {
    //        char err[DRIVER_LEN] = {0};
...
bool DomePro2::calibrateDomeAz(double az)
{
    char cmd[DRIVER_LEN] = {0};
    uint32_t steps = static_cast<uint32_t>(az * (SettingsNP[SETTINGS_AZ_CPR].value / 360));
    //    if (steps >= SettingsNP[SETTINGS_AZ_CPR].getValue())
    //    {
    //        char err[DRIVER_LEN] = {0};
