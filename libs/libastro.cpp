...
void LibAstro::ObservedToJ2000(ln_equ_posn * observed, double jd, ln_equ_posn * J2000pos)
{
    ...
    // with the nutation removed instead of added
    struct ln_nutation nut;
    ln_get_nutation (jd, &nut);

    double mean_ra, mean_dec, delta_ra, delta_dec;

    mean_ra = ln_deg_to_rad(tempPos.ra);
    mean_dec = ln_deg_to_rad(tempPos.dec);
    ...
    tempPos.ra = tempPos.ra - delta_ra;
    tempPos.dec = tempPos.dec - delta_dec;
